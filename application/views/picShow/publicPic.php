...
<body>
	...
	<div class="mainbox">
		<div class="phbut left"><a href="javascript:void(0)" style="background:#ff0000;">共有フォルダ</a></div>	
		...
		<div class="maintable maintable26">
			...
			<input type="hidden" name="hidden_sel" value="Pub"/>
			<div class="taimgbox">
				<ul class="phfile">
				...
				<ul class="phimg">
					...
					?>
					<li class="draggable" fileName="<?php echo $val_no['name'];?>"><div style="width: 115px;height:115px"><a javascript><img style="width:110px" src="<?php $relative_path=iconv("shift-jis","UTF-8", $val_no['relative_path']); echo $media.$relative_path;?>"/></a></div><p><a javascript class="fileName"><?php echo $val_no['name'];?></a></p></li>					
					<?php }}?>
				</ul>			
				<div class="clear"></div>
			</div>
	...
	$(function(){
		...
		$('.file_dbClick').dblclick(function(){
			...
			window.location.href="<?php echo URL::site('picshow/publicPic').URL::query(array('dir'=>null,'sort'=>null,'pre_dir'=>null));?>?dir="+new_dir+"&pre_dir="+old_dir;
		...
		$('.back_dir').dblclick(function(){
			...
			window.location.href="<?php echo URL::site('picshow/publicPic').URL::query(array('dir'=>null,'sort'=>null,'pre_dir'=>null));?>?dir="+new_dir+"&pre_dir="+pre_dir;			
		});
		...
		$( ".draggable" ).draggable({ 
			...
			stop:function(event,ui){
				 ...
				 var data = 'moveFile='+moveFile+'&aimFile='+aimFile+'&sel=Pub';
				 $.ajax({
					   type: "POST",
	...
	function overlayRemovePic()
		{	
			...
			window.location.href="<?php echo URL::site('picshow/publicPic').URL::query(array('dir'=>null,'sort'=>null,'pre_dir'=>null));?>?dir="+dir+"&pre_dir="+old_dir;
		}
	...
	function sort(tag){
		var dir=$('input[name="hidden_dir"]').val();
		var old_dir=$('input[name="hidden_pre_dir"]').val();
		window.location.href="<?php echo URL::site('picshow/publicPic').URL::query(array('dir'=>null,'sort'=>null,'pre_dir'=>null));?>?dir="+dir+'&sort='+tag+"&pre_dir="+old_dir;
	}
	...
	function delFile(){
		...
		var data = "delFile="+dir_short+'&sel=Pub';
		$.ajax({
			   type: "POST",
	...
	function saveFileName(){
		...
		var data ='fileName='+fileName+'&dir_short='+dir_short+'&sel=Pub';
		$.ajax({
			   type: "POST",
	...
	function updateFileName(){
		...
		var dir_short=$('input[name="hidden_dir"]').val();
		var data ='newFileName='+newFileName+'&dir_short='+dir_short+'&oldFileName='+oldFileName+'&sel=Pub';
		$.ajax({
			   type: "POST",
