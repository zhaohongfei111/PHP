...
<body class="bg01">
	...
	<form id="formMain">
		...
		<div class="maintablebox">
			<div class="xinnavbox">
				<span>表示切替：</span><em class="a1"><a href="<?php echo URL::site('administration/capitalSet').URL::query(array('Status'=>'in'));?>" <?php echo $status=='in'?'class="on"':'';?>>在園児</a></em><em class="a2"><a href="<?php echo URL::site('administration/capitalSet').URL::query(array('Status'=>'not'));?>" <?php echo $status=='not'?'class="on"':'';?>>入園前の園児</a></em>
			</div>
                  		...
                  		<?php foreach ($list as $key=>$val){
                  			if($val['Recog_Type']==$key1){?>
                  				<tr>
									...
									<td><input name="txt_Capital_ID[<?php echo $val['ID']?>]" type="text" class="txt02" value="<?php echo $val['Capital_ID']?>"/></td>
                  				</tr>
                  				<?php }}?>
                    </tbody>
                    <?php }?>			
	...
	function formMainSave(){			
		...
		$.ajax({
			   type: "POST",
			   url: "<?php echo URL::site('administration/capitalSet_Insert');?>",
			   cache: false,
			   dataType: 'json',
