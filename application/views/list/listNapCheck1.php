...
<body>
	...
	<div class="mainbox">
		...
		<div class="maintablebox">
			...
			<div class="maintable maintable18">
            	<?php
                if($SELLEVEL!='Reader'){
				?>
				<div class="mbut18">
					<a href="javascript:void(0)" onClick="formSubmit()" style="padding:0 150px;">保 存 す る</a>
					<a href="javascript:void(0)" onClick="addColspan()" style="background-color: #FF0000">睡眠時間チェック枠の追加</a>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="table-layout:fixed;">
                    ...
                    <tbody id="<?php echo 'tab_con_'.$key1?>" <?php if($key1!='C1'){echo 'style="display:none"';}else {echo 'style="display:table-row-group"';}?> >
                        ...
                        foreach ($child_Info as $key=>$val){
                            if($val['Class']==$key1){
								$num_tr=count($val['Check_list'])<count($val['Sleep_list'])?count($val['Sleep_list']):count($val['Check_list']);
                        ...
                        <tr>
                            <td class="tt01">
                            	...
                            	<input type="hidden" name="txt_Nap_ID[<?php echo $val['ID'];?>][]" value="<?php echo array_key_exists($tr, $val['Check_list'])?$val['Check_list'][$tr]['ID']:''?>">
                            	<input type="hidden" name="txt_Sleep_ID[<?php echo $val['ID'];?>][]" value="<?php echo array_key_exists($tr, $val['Sleep_list'])?$val['Sleep_list'][$tr]['ID']:''?>">
                            </td>
                           ...
                           <!-- 就床与起床 -->							
                            <td>                        	
                            	<input name="txt_Begin_Sleep_Time[<?php echo $val['ID'];?>][<?php echo $tr+1;?>]" type="text" style="border:1px solid #FF0000" class="txt01" value="<?php echo array_key_exists($tr, $val['Sleep_list'])?$val['Sleep_list'][$tr]['Begin_Sleep_Time']:''?>">
                            </td>
                            <td class="tag"><input name="txt_End_Sleep_Time[<?php echo $val['ID'];?>][<?php echo $tr+1;?>]" type="text" style="border:1px solid #FF0000" class="txt01" value="<?php echo array_key_exists($tr, $val['Sleep_list'])?$val['Sleep_list'][$tr]['End_Sleep_Time']:''?>"></td>
...
function addColspan(){
	...
	var new_Tr  = '<tr style="height:'+height_Tr+'px"><td class="tt01"><input type="hidden" name="txt_Nap_ID['+child_id+'][]" value=""><input type="hidden" name="txt_Sleep_ID['+child_id+'][]" value=""> </td>';
		new_Tr += '	<td> </td>';
